...
BOOL CNetManager::WorkThread_Listen()
{
    ...
    while(TRUE)
    {
        ...
        {
            spdlog::error("accept 错误 原因:{}!", CommonFunc::GetLastErrorStr(CommonSocket::GetSocketLastError()).c_str());
            return FALSE;
        }
        ...
        {
            spdlog::error("接受新连接失败 原因:己达到最大连接数或者绑定失败!");
        }
    }
...
BOOL CNetManager::StartNetListen(UINT16 nPortNum, std::string strIpAddr)
{
    ...
    {
        spdlog::error("创建监听套接字失败原因:{}!", CommonFunc::GetLastErrorStr(CommonSocket::GetSocketLastError()).c_str());
        return FALSE;
    }
    ...
    {
        spdlog::error("邦定套接字失败原因:{}!", CommonFunc::GetLastErrorStr(CommonSocket::GetSocketLastError()).c_str());
        return FALSE;
    }

    if(!CommonSocket::ListenSocket(m_hListenSocket, 20))
    {
        spdlog::error("监听线程套接字失败:{}!", CommonFunc::GetLastErrorStr(CommonSocket::GetSocketLastError()).c_str());
        return FALSE;
    }

    if (!WaitForConnect())
    {
        spdlog::error("等待接受连接失败:{}!", CommonFunc::GetLastErrorStr(CommonSocket::GetSocketLastError()).c_str());
        return FALSE;
    }
...
BOOL CNetManager::WorkThread_ProcessEvent(INT32 nParam)
{
    ...
    while(!m_bCloseEvent)
    {
        ...
        if(!bRetValue)
        {
            ...
            //  spdlog::error(" GetQueuedCmpletionStatus Error: %d-%d-{}", pIoPeratorData->nOpType, CommonFunc::GetLastError(), CommonFunc::GetLastErrorStr(CommonFunc::GetLastError()).c_str());
            //}
        }
        ...
        switch( pIoPeratorData->nOpType)
        {
            case NET_OP_RECV:
            {
                ...
                if(pConnection == NULL)
                {
                    spdlog::error("触发了NET_MSG_RECV1, pConnection == NULL");
                    break;
                }
                ...
                if(nNumOfByte == 0)
                {
                    ...
                    {
                        spdlog::error("触发了NET_MSG_RECV2, 对方己经关闭连接，但可能我们这边更快, 连接己经被重用了。NewID:%d, OrgID:%d", pConnection->GetConnectionID(), pIoPeratorData->nConnID);
                        break;
                    }
                    pConnection->Close();
                }
                else
                {
                    if(pConnection->GetConnectionID() != pIoPeratorData->nConnID)
                    {
                        spdlog::error("触发了NET_MSG_RECV3，确实有数据, 但连接己经被重用了。NewID:%d, OrgID:%d", pConnection->GetConnectionID(), pIoPeratorData->nConnID);
                        break;
                    }
                    ...
                    {
                        spdlog::error("严重错误, 没有连接上，却收到的数据! ConnID:{}, ConnectStatus:{}", pConnection->GetConnectionID(), pConnection->GetConnectStatus());
                    }
                }
            }
            break;
            case NET_OP_SEND:
            {
                ...
                if (pConnection == NULL)
                {
                    spdlog::error("触发了NET_MSG_SEND, pConnection == NULL。");
                    break;
                }

                if(pConnection->GetConnectionID() != pIoPeratorData->nConnID)
                {
                    spdlog::error("触发了NET_MSG_SEND, 但连接己经被关闭重用了。");
                    break;
                }
            ...
            case NET_OP_POST:
            {
                ...
                if (pConnection == NULL)
                {
                    spdlog::error("触发了NET_MSG_POST1, pConnection == NULL。");
                    break;
                }

                if (pConnection->GetConnectionID() != pIoPeratorData->nConnID)
                {
                    spdlog::error("触发了NET_MSG_POST2, 但连接己经被关闭重用了。");
                    break;
                }
            ...
            case NET_OP_CONNECT:
            {
                ...
                if (pConnection == NULL)
                {
                    spdlog::error("触发了NET_MSG_CONNECT, pConnection == NULL。");
                    break;
                }

                if (pConnection->GetConnectionID() != pIoPeratorData->nConnID)
                {
                    spdlog::error("触发了NET_MSG_CONNECT, 事件ID和连接ID不一致。");
                    break;
                }
            ...
            case NET_OP_ACCEPT:
            {
                ...
                {
                    spdlog::error("接受新连接失败 原因:GetSocketAddress FALSE Reason:{}!", CommonFunc::GetLastErrorStr(CommonSocket::GetSocketLastError()).c_str());
                    break;
                }
                ...
                {
                    spdlog::error("接受新连接失败 原因:AssociateCompletePort己达到最大连接数或者绑定失败!");
                }
...
CConnection* CNetManager::AssociateCompletePort( SOCKET hSocket, BOOL bConnect)
{
    ...
    {
        spdlog::error("AssociateCompletePort失败!");
        pConnection->Close();
    }
...
BOOL CNetManager::WorkThread_ProcessEvent(INT32 nParam)
{
    ...
    while (!m_bCloseEvent)
    {
        ...
        if (nFd == -1)
        {
            if (errno != EINTR)
            {
                spdlog::error("epoll_wait失败 原因:{}", CommonFunc::GetLastErrorStr(errno).c_str());
                return FALSE;
            }
        ...
        for (int i = 0; i < nFd; ++i)
        {
            if (vtEvents[i].data.fd == m_hListenSocket)
            {
                ...
                while (TRUE)
                {
                    ...
                    if (hClientSocket == INVALID_SOCKET)
                    {
                        ...
                        {
                            spdlog::error("接受新连接失败 原因:{}", CommonFunc::GetLastErrorStr(errno).c_str());
                        }
            ...
            if (pConnection == NULL)
            {
                spdlog::error("---Invalid pConnection Ptr--!");
                continue;
            }
            ...
            if (nNeedEvent != 0)
            {
                ...
                {
                    spdlog::error("socket 设置事件失败 原因:{}", CommonFunc::GetLastErrorStr(errno).c_str());
                }
            }
        }
    }
...
BOOL CNetManager::EventDelete(CConnection* pConnection)
{
    ...
    {
        spdlog::error("---EventDelete::epoll_ctl 失败 原因:{}!", CommonFunc::GetLastErrorStr(errno).c_str());
        return FALSE;
    }
...
BOOL CNetManager::Start(UINT16 nPortNum, INT32 nMaxConn, IDataHandler* pBufferHandler, std::string strListenIp)
{
    ...
    if(!InitNetwork())
    {
        spdlog::error("初始化网络失败！！");
        return FALSE;
    }

    if(!CreateCompletePort())
    {
        spdlog::error("创建完成端口或Epoll失败！！");
        return FALSE;
    }

    if(!CreateEventThread(0))
    {
        spdlog::error("创建网络事件处理线程失败！！");
        return FALSE;
    }
...
CConnection* CNetManager::ConnectTo_Async( std::string strIpAddr, UINT16 sPort )
{
    ...
    if(!bRet)
    {
        spdlog::error("ConnectTo_Async 连接目标服务器失败,IP:{}--Port:%d!!", strIpAddr.c_str(), sPort);
        pConnection->Close();
    }
    ...
    if (!bRet)
    {
        spdlog::error("ConnectTo_Async 连接失败,IP:{},Port:%d!", strIpAddr.c_str(), sPort);
        CommonSocket::CloseSocket(hSocket);
        return NULL;
    }
...
BOOL CNetManager::WaitForConnect()
{
    ...
    {
        spdlog::error("WaitConnect邦定Listen套接字失败:{}!", CommonFunc::GetLastErrorStr(CommonSocket::GetSocketLastError()).c_str());
        return FALSE;
    }
    m_IoOverlapAccept.Reset();
    m_IoOverlapAccept.nOpType = NET_OP_ACCEPT;
...
BOOL CNetManager::SendMessageData(INT32 nConnID,  INT32 nMsgID, UINT64 u64TargetID, UINT32 dwUserData,  const char* pData, INT32 nLen)
{
    ...
    {
        spdlog::error("CNetManager::SendMessageData FAILED, 连接己断开, MsgID:%d, nConnID:%d", nMsgID, nConnID);
        return FALSE;
    }
...
BOOL CNetManager::SendMessageBuff(INT32 nConnID, IDataBuffer* pBuffer)
{
    ...
    if(pConn->GetConnectStatus() != ENS_CONNECTED)
    {
        spdlog::error("CNetManager::SendMessageBuff FAILED, 连接己断开, ConnID:%d", nConnID);
        return FALSE;
    }
